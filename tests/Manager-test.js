...
test('Acquires Employee properties', () => {
    const manager = new Manager('Shinji', 01, 'shinji@seele.com');
  ...
  test('Overwrites Employee role (Manager)', () => {
    const manager = new Manager('Asuka', 789, 'Toji@email.com', 2);
    expect(manager.getRole()).toBe('Manager');
  });

  test('Overwrites Employee icon based on role (Manager)', () => {
    const manager = new Manager('Toji', 100, 'Toji@email.com', 3);
